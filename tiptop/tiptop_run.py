...
def process_scene_geometry(
    ...
    gripper_mesh = get_gripper_mesh_in_tcp_frame(robot_type=str(tiptop_cfg().robot.type))
...
def log_raw_m2t2_results(depth_results: dict, max_grasps: int = 100) -> None:
    ...
    for group_name, grasp_data in depth_results["grasps"].items():
        ...
        # Poses are world_from_m2t2_grasp. Convert them to the Cobot TCP
        # convention, but deliberately do not apply object association/filtering.
        world_from_tcp = poses @ m2t2_grasp_from_tcp()
        transformed_vertices = np.einsum("nij,mj->nmi", world_from_tcp, vertices_hom)[..., :3]
